...
.PHONY: help install-deps native-install native-upgrade check test build-updater update rebuild rebuild-install inspect-upstream build-app rebuild-next run-app build-dev-app run-dev-app deb rpm pacman package install service-enable service-status clean-dist clean-state
...
native-install:
	...
	@if [ -n "$(strip $(DMG))" ]; then \
		./scripts/one-command-install.sh "$(DMG)"; \
	else \
		./scripts/one-command-install.sh; \
	fi

native-upgrade:
	@echo "[make] Rebuilding and reinstalling native package from a fresh DMG"
	@if [ -n "$(strip $(DMG))" ]; then \
		./scripts/one-command-install.sh --fresh "$(DMG)"; \
	else \
		./scripts/one-command-install.sh --fresh; \
	fi
